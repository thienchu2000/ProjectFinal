const WordBestNFT_two = artifacts.require("WordBestNFT_two");
...
module.exports = async function (deployer, network, accounts) {
  ...
  await deployer.deploy(WordBestNFT_two);

  const WordBestNFT_twoInstance = await WordBestNFT_two.deployed();
  for (const cid of ipfsCIDs) {
    await WordBestNFT_twoInstance.awardItem(initialOwner, cid);
  }
};
